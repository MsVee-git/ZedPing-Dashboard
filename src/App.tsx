...
function Contacts({ customer }) {
  ...
  return <div className="pad" style={{ padding: 28 }}>
    ...
    <PageHead label="Database" title="Contacts." sub={loading ? "Loading..." : `${data?.length || 0} contacts`} action={canManageGroups ? (tab === "groups" ? <button className="btn btn-gold" onClick={() => setShowGroupForm(true)}><Ic n="plus" s={12} c="var(--ink)" />New Group</button> : <button className="btn btn-gold" onClick={() => setShowImport(true)}><Ic n="plus" s={12} c="var(--ink)" />Import Contacts</button>) : null} />
    ...
    {activeGroup && <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.7)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}><div className="dialog-surface" style={{background:"var(--panel)",padding:28,width:"100%",maxWidth:620,maxHeight:"80vh",overflow:"auto"}}><div className="mono" style={{fontSize:9,color:"var(--gold2)",letterSpacing:2}}>CONTACT LIST</div><h3 className="editorial" style={{color:"var(--cream)",fontSize:24}}>{activeGroup.name}</h3>{canManageGroups && <div style={{display:"flex",gap:8,margin:"16px 0"}}><select className="input" value={selectedContactId} onChange={event=>setSelectedContactId(event.target.value)}><option value="">Select an existing contact…</option>{(data||[]).filter(contact=>!members.some(member=>member.contact_id===contact.id)).map(contact=><option key={contact.id} value={contact.id}>{contact.name} · {contact.phone_number}</option>)}</select><button className="btn btn-gold" onClick={addMember} disabled={!selectedContactId}>Add</button></div>}{!members.length?<Empty msg="No contacts in this list yet"/>:<div className="card">{members.map(member=><div key={member.id} className="row" style={{gridTemplateColumns:"2fr 1.5fr 80px",gap:12}}><div style={{color:"var(--cream)",fontSize:13}}>{member.contact.name}</div><div style={{color:"var(--mist)",fontSize:12}}>{member.contact.phone_number}</div>{canManageGroups ? <button className="btn btn-wire" style={{fontSize:9,padding:"5px 8px"}} onClick={()=>removeMember(member)}>Remove</button> : <span className="mono" style={{fontSize:9,color:"var(--mist)"}}>Member</span>}</div>)}</div>}<div style={{display:"flex",justifyContent:"flex-end",marginTop:18}}><button className="btn btn-wire" onClick={()=>setActiveGroup(null)}>Done</button></div></div></div>}    <ContactsImport open={showImport} onClose={() => setShowImport(false)} apiFetch={apiFetch} apiBase={API} onImported={() => { refetch(); if (tab === "groups") loadGroups(); }} />
