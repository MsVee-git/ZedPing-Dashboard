...
function Automations({ customer }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const payload = legacy ? { trigger_type: "keyword", trigger_value: "DEFAULT", message_template: form.response, priority: Number(form.priority) } : {
        ...
        trigger_config: ["keyword","faq","human_handoff"].includes(composer) ? { phrases: list, ...(composer === "faq" && form.topic.trim() ? { topic: form.topic.trim() } : {}) } : {},
  ...
  const lastFor = (automationId) => (historyData || []).find((event) => event.automation_id === automationId)?.created_at || null;
  ...
  return <div className="pad" style={{ padding: 28, maxWidth: 1240, margin: "0 auto" }}>
    ...
    <section className="card" style={{ marginBottom: 22 }}><div style={{ padding: "18px 20px", borderBottom: "1px solid var(--wire)", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}><div><div style={{ fontSize: 16, fontWeight: 600 }}>Your Automations</div><div style={{ fontSize: 12, color: "var(--mist)", marginTop: 4 }}>Only rules belonging to this workspace are shown.</div></div><button className="btn btn-wire" onClick={refetch} disabled={loading}><Ic n="refresh" s={12} /> Refresh</button></div>
      ...
      {loading ? <Loader /> : error ? <div role="alert" style={{ padding: 24, color: "var(--error-text)", fontSize: 13 }}>We could not load automations. <button className="btn btn-wire" onClick={refetch} style={{ marginLeft: 8 }}>Try again</button></div> : !automations.length ? <div style={{ textAlign: "center", padding: "52px 24px" }}><div style={{ fontSize: 20 }}>Put your WhatsApp on autopilot</div><div style={{ color: "var(--mist)", fontSize: 13, margin: "9px auto 18px", maxWidth: 420 }}>Start with a ready-made automation and customize it for your business.</div>{canManage && <button className="btn btn-gold" onClick={() => openComposer("keyword")}>Create your first response</button>}</div> : <div><div className="row th hide-m" style={{ gridTemplateColumns: "1.3fr 2.4fr 100px 86px 116px", gap: 14 }}><div>Automation</div><div>What it does</div><div>Status</div><div>Priority</div><div /></div>{automations.map((item) => <div key={item.id} className="row" style={{ gridTemplateColumns: "1.3fr 2.4fr 100px 86px 116px", gap: 14 }}><div><div style={{ fontWeight: 600 }}>{nameFor(item)}</div><div style={{ color: "var(--mist)", fontSize: 11, marginTop: 4 }}>{typeFor(item)} · {lastFor(item.id) ? "Last activity " + when(lastFor(item.id)) : "No activity yet"}</div></div><div style={{ color: "var(--cream2)", fontSize: 12, lineHeight: 1.4 }}>{summaryFor(item)}</div><div><span className={"badge " + (item.is_active ? "badge-green" : "badge-cream")}>{item.is_active ? "Active" : "Paused"}</span></div><div className="mono" style={{ color: "var(--mist)", fontSize: 11 }}>{item.priority ?? 100}</div><div style={{ display: "flex", justifyContent: "flex-end", gap: 7 }}>{canManage && <><button className="btn btn-wire" onClick={() => openComposer(item.automation_type || "keyword", item)} style={{ padding: "6px 9px", fontSize: 8 }}>Edit</button><button className="btn btn-wire" onClick={() => toggle(item)} style={{ padding: "6px 9px", fontSize: 8 }}>{item.is_active ? "Pause" : "Activate"}</button></>}</div></div>)}</div>}
    </section>
