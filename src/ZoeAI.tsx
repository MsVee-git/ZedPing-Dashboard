...
export function ZoeAI({ customer, apiFetch }) {
  ...
  async function save() {
    try {
      ...
      setDraft(null); setSelected(result.agent); await load(); setNotice("Draft saved. It is not active on WhatsApp.");
    } catch(error) { setNotice(error?.message || "We could not save this draft."); }
  }
  ...
  async function addTestContact() {
    ...
    try {
      ...
      const result=await response.json();
      setNotice("Approved test contact added for controlled activation."); setTestContactPhone("");
      return result;
  ...
  async function activate() {
    try {
      ...
      const result=await response.json();
      setSelected(result.agent); setActivationReview(null); await load(); setNotice("Active on WhatsApp. Only approved test contacts can receive controlled Zoe responses.");
  ...
  async function pause() {
    ...
    try {
      ...
      const result=await response.json(); setSelected(result.agent); await load(); setNotice("Agent paused. Existing conversations remain in Team Inbox.");
  ...
  if (draft) {
    ...
    return <div style={{padding:"28px 32px",maxWidth:1000,margin:"0 auto"}}><h1 className="editorial" style={{fontSize:42}}>Zoe AI</h1><p style={{color:"var(--cream2)"}}>Configure a private draft. It will not respond on WhatsApp.</p>{notice && <p role="status">{notice}</p>}
      ...
      <button className="btn btn-gold" style={{marginTop:20}} disabled={!draft.assistant_name || !draft.whatsapp_number_id} onClick={save}>Save draft</button> <button className="btn btn-wire" onClick={()=>setDraft(null)}>Back</button></div>
