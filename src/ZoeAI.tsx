...
export function ZoeAI({ customer, apiFetch }) {
  ...
  if (selected) return <div style={{padding:"28px 32px",maxWidth:1000,margin:"0 auto"}}><h1 className="editorial" style={{fontSize:42}}>{selected.name}</h1><p style={{color:"var(--gold2)"}}>Draft — not active on WhatsApp.</p>{notice && <p role="status">{notice}</p>}<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,marginTop:20}}><div style={{padding:20,border:"1px solid var(--wire)",background:"var(--panel)"}}><p><strong>Knowledge sources:</strong> {selected.knowledge?.length ? selected.knowledge.map(item=>item.name).join(", ") : "None selected"}</p><p><strong>Communication:</strong> {selected.configuration?.communication_style || "professional"}</p>{canManage && <><button className="btn btn-gold" onClick={()=>start({},selected)}>Edit draft</button> <button className="btn btn-wire" onClick={archive}>Archive</button></>} <button className="btn btn-wire" onClick={()=>setSelected(null)}>Back</button></div><div style={{padding:20,border:"1px solid var(--wire)",background:"var(--panel)"}}><div className="mono">PRIVATE TEST AGENT</div>{messages.map((entry,index)=><p key={index}><strong>{entry.role==="user"?"You":selected.name}:</strong> {entry.content}</p>)}{test && <p style={{color:"var(--cream2)"}}>{test.no_approved_knowledge ? "No approved knowledge matched" : "Knowledge used: "+(test.knowledge_used||[]).join(", ")}<br/>{test.would_handoff ? "Would hand to team" : "Would continue privately"}</p>}{canManage && <><textarea className="textarea" value={question} onChange={e=>setQuestion(e.target.value)} placeholder="Ask a customer question…"/><button className="btn btn-gold" onClick={testAgent}>Test Agent</button> <button className="btn btn-wire" onClick={()=>{setMessages([]);setTest(null)}}>New test</button></>}</div></div></div>;
