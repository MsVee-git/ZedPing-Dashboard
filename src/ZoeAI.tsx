...
export function ZoeAI({ customer, apiFetch, routeAgentId = null, onRouteOpen, onRouteUnavailable }) {
  ...
  async function activate() {
    try {
      ...
      const result=await response.json();
      openAgent(result.agent); setActivationReview(null); await load(); setNotice("Active on WhatsApp â Test contacts only.");
    } catch(error) { setNotice(error?.message || "We could not activate this agent."); }
  }
  async function goLive() {
    try { const response=await apiFetch("/ai-agents/"+selected.id+"/go-live",{method:"POST"}); const result=await response.json(); openAgent(result.agent); setActivationReview(null); await load(); setNotice("Live on WhatsApp â Responding to eligible customers."); }
  ...
  async function returnToTest() {
    ...
    try { const response=await apiFetch("/ai-agents/"+selected.id+"/test-mode",{method:"POST"}); const result=await response.json(); openAgent(result.agent); await load(); setNotice("Testing on WhatsApp â Approved test contacts only."); }
  ...
  async function resume() {
    ...
    try { const response=await apiFetch("/ai-agents/"+selected.id+"/resume",{method:"POST"}); const result=await response.json(); openAgent(result.agent); await load(); setNotice(result.agent?.deployment_mode === "live" ? "Live on WhatsApp â Responding to eligible customers." : "Testing on WhatsApp â Approved test contacts only."); }
  ...
  if (draft) {
    ...
    return <div style={{padding:"28px 32px",maxWidth:1000,margin:"0 auto"}}><h1 className="editorial" style={{fontSize:42}}>Zoe AI</h1><p style={{color:"var(--cream2)"}}>Configure a private draft. It will not respond on WhatsApp.</p>{notice && <p role="status">{notice}</p>}
      ...
      <label className="label" style={{marginTop:16}}>Approved knowledge (up to 5)</label>{knowledge.length ? knowledge.map(item=><label key={item.id} style={{display:"block",padding:"7px 0"}}><input type="checkbox" checked={draft.knowledge_item_ids.includes(item.id)} onChange={()=>toggleKnowledge(item.id)}/> {item.name} Â· {item.source_type === "image" ? "Image" : "Text"}</label>) : <p style={{color:"var(--mist)"}}>No eligible approved knowledge yet. Review and approve an Image extraction in Content Library before selecting it here.</p>}
      ...
      <button className="btn btn-gold" style={{marginTop:20}} disabled={saving || !draft.assistant_name || !draft.whatsapp_number_id} onClick={save}>{saving ? "Savingâ¦" : "Save draft"}</button> <button className="btn btn-wire" onClick={()=>setDraft(null)}>Back</button></div>
      <aside style={{padding:20,border:"1px solid var(--wire)",background:"var(--panel)"}}><div className="mono">REVIEW</div><h2 className="editorial" style={{fontSize:30,marginTop:10}}>{draft.assistant_name || "Your assistant"}</h2><p style={{marginTop:10}}>{current?.role}</p><p style={{marginTop:14}}><strong>Can help with:</strong> {current?.can}</p><p><strong>Hands to the team:</strong> {current?.handoff}</p><p><strong>Cannot do:</strong> {current?.unavailable}</p><p style={{color:"var(--gold2)",marginTop:16}}>Status: Draft â not active on WhatsApp.</p></aside></div></div>;
  }
  if (selected) return <div style={{padding:"28px 32px",maxWidth:1000,margin:"0 auto"}}>{activationReview && <div role="dialog" aria-modal="true" style={{position:"fixed",inset:0,zIndex:30,background:"rgba(0,0,0,.65)",display:"grid",placeItems:"center",padding:20}}><div style={{maxWidth:560,padding:24,border:"1px solid var(--wire)",background:"var(--panel)"}}><h2 className="editorial" style={{fontSize:30}}>{selected?.lifecycle_status === "active" ? `Go Live with ${activationReview.assistant_name}?` : `Activate ${activationReview.assistant_name}?`}</h2><p>{selected?.lifecycle_status === "active" ? "Once live, this assistant can respond to eligible incoming WhatsApp messages using the activated approved information. Conversations can be handed to your team based on your handoff settings." : "Once activated, this assistant can respond to eligible incoming WhatsApp messages using the approved information you selected. Conversations can be handed to your team based on your handoff settings."}</p><p><strong>WhatsApp number:</strong> {numbers.find(item=>item.id===activationReview.whatsapp_number_id)?.phone_number || "Connected workspace number"}</p><p><strong>Knowledge:</strong> {(activationReview.knowledge_sources||[]).join(", ")}</p><p><strong>Handoff:</strong> customer requests, unavailable approved information, and quote or purchase requests as configured.</p><button className="btn btn-gold" onClick={selected?.lifecycle_status === "active" ? goLive : activate}>{selected?.lifecycle_status === "active" ? "Go Live" : "Activate Agent"}</button> <button className="btn btn-wire" onClick={()=>setActivationReview(null)}>Cancel</button></div></div>}<h1 className="editorial" style={{fontSize:42}}>{selected.name}</h1><p style={{color:"var(--gold2)"}}>{selected.lifecycle_status==="active" ? (selected.deployment_mode === "live" ? "Live on WhatsApp â Responding to eligible customers Â· " : "Testing on WhatsApp â Approved test contacts only Â· ")+(numbers.find(item=>item.id===selected.whatsapp_number_id)?.phone_number || "connected number") : selected.lifecycle_status==="paused" ? "Paused â not responding to WhatsApp." : "Draft â not active on WhatsApp."}</p>{notice && <p role="status">{notice}</p>}<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,marginTop:20}}><div style={{padding:20,border:"1px solid var(--wire)",background:"var(--panel)"}}><p><strong>Knowledge sources:</strong> {selected.knowledge?.length ? selected.knowledge.map(item=>item.name).join(", ") : "None selected"}</p><p><strong>Communication:</strong> {selected.configuration?.communication_style || "professional"}</p>{canManage && <><label className="label" style={{marginTop:16}}>Approved test contacts</label>{testContacts.length ? testContacts.map(contact=><div key={contact.id} style={{display:"flex",justifyContent:"space-between",gap:10,padding:"7px 0"}}><span>{contact.phone_e164}</span>{selected.lifecycle_status!=="active" && <button className="btn btn-wire" onClick={()=>removeTestContact(contact)}>Remove</button>}</div>) : <p style={{color:"var(--mist)"}}>No approved test contacts yet.</p>}</>}{canManage && selected.lifecycle_status!=="active" && <><input className="input" value={testContactPhone} onChange={e=>setTestContactPhone(e.target.value)} placeholder="+260â¦"/><button className="btn btn-wire" onClick={addTestContact}>Add test contact</button><button className="btn btn-gold" disabled={!testContacts.length} onClick={reviewActivation}>Activate Agent</button><button className="btn btn-wire" onClick={()=>start({},selected)}>Edit draft</button> <button className="btn btn-wire" onClick={archive}>Archive</button></>}{canManage && selected.lifecycle_status==="active" && <><button className="btn btn-gold" onClick={pause}>Pause Agent</button>{selected.deployment_mode === "test" && <button className="btn btn-wire" onClick={reviewActivation}>Go Live</button>}{selected.deployment_mode === "live" && <button className="btn btn-wire" onClick={returnToTest}>Return to Test Mode</button>}</>}{canManage && selected.lifecycle_status==="paused" && <button className="btn btn-gold" onClick={resume}>Resume {selected.deployment_mode === "live" ? "Live" : "Test Mode"}</button>} <button className="btn btn-wire" onClick={()=>setSelected(null)}>Back</button></div><div style={{padding:20,border:"1px solid var(--wire)",background:"var(--panel)"}}><div className="mono">PRIVATE TEST AGENT</div>{messages.map((entry,index)=><p key={index}><strong>{entry.role==="user"?"You":selected.name}:</strong> {entry.content}</p>)}{test && <p style={{color:"var(--cream2)"}}>{test.no_approved_knowledge ? "No approved knowledge available" : "Approved knowledge available: "+(test.approved_knowledge_available||[]).join(", ")}<br/>{test.would_handoff ? "Would hand to team" : "Would continue privately"}</p>}{canManage && <><textarea className="textarea" value={question} onChange={e=>setQuestion(e.target.value)} placeholder="Ask a customer questionâ¦"/><button className="btn btn-gold" onClick={testAgent}>Test Agent</button> <button className="btn btn-wire" onClick={()=>{setMessages([]);setTest(null)}}>New test</button></>}</div></div></div>;
  return <div style={{padding:"28px 32px",maxWidth:1180,margin:"0 auto"}}><div style={{display:"flex",justifyContent:"space-between"}}><div><div className="mono" style={{color:"var(--gold2)"}}>ZOE AI</div><h1 className="editorial" style={{fontSize:42}}>AI Agents</h1><p style={{color:"var(--cream2)"}}>Create private draft assistants from selected approved knowledge.</p></div>{canManage && <button className="btn btn-gold" onClick={()=>start(templates[0]||{key:"common_questions"})}>Create AI Agent</button>}</div>{notice && <p role="status">{notice}</p>}<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:14,marginTop:22}}>{templates.map(item=><div key={item.key} style={{padding:18,border:"1px solid var(--wire)",background:"var(--panel)"}}><h2 className="editorial" style={{fontSize:26}}>{item.title}</h2><p>{item.role}</p><p style={{color:"var(--mist)"}}>Hands to a person: {item.handoff}</p>{canManage && <button className="btn btn-gold" onClick={()=>start(item)}>Use template</button>}</div>)}</div><h2 className="editorial" style={{fontSize:32,marginTop:28}}>My Agents</h2>{agents.map(agent=><button key={agent.id} className="slink" onClick={()=>openAgent(agent)}>{agent.name} Â· {agent.lifecycle_status} Â· {agent.knowledge?.length || 0} approved knowledge item(s)</button>)}</div>;
}
