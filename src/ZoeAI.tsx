...
export function ZoeAI({ customer, apiFetch, routeAgentId = null, onRouteOpen, onRouteUnavailable }) {
  ...
  async function resume() {
    ...
    try { const response=await apiFetch("/ai-agents/"+selected.id+"/resume",{method:"POST"}); const result=await response.json(); openAgent(result.agent); await load(); setNotice(result.agent?.deployment_mode === "live" ? "Live on WhatsApp — Responding to eligible customers." : "Testing on WhatsApp — Approved test contacts only."); }
