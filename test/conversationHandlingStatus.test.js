...
test('list, header and details share rendering while existing Inbox action endpoints and reply guards remain', () => {
  ...
  for (const action of ['take', 'resolve', 'reopen']) assert.ok(inbox.includes(`runAction('/${action}'`))
  assert.match(inbox, /const canReply=isHuman && \(isAssignedToMe \|\| canManageAssignment\)/)
